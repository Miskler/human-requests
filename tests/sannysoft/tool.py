...
def select_unexpected_failures(
    ...
    *,
    ...
    - если v.passed == False, но этот ключ НЕ входит в declared stable/unstable
      (для данного stealth или для all),
      то добавляем в список как «неожиданный фейл»;
    - если v.passed == True, а ключ объявлен «stable»,
      то добавляем маркер, что ожидался фейл («(shld, bt not f)»).

    Возвращает список строковых «путей» (с префиксами через " → ")
    для красивого сообщения об ошибке.
    """
    fails: list[str] = []
