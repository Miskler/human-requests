...
from network_manager import Session, HttpMethod, ImpersonationConfig
...
async def session() -> Session:
    s = Session(
        spoof=ImpersonationConfig(sync_with_engine=False)  # любой профиль
    )
    yield s
    await s.close()
...
async def test_httpbin_headers_echo(session: Session):
    ...
    echoed = {
        k.lower(): v for k, v in json.loads(resp.body)["headers"].items()
    }
